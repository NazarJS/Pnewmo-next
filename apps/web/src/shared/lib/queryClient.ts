  ...
  return new QueryClient({
    defaultOptions: {
      ...
      dehydrate: {
        ...
        // Сейчас недостижимо: shouldRedactErrors применяется только при
        // дегидратации ошибок и незавершённых (pending) запросов, а
        // shouldDehydrateQuery выше сузили до только успешных — записи,
        // которую можно было бы редактировать, просто не бывает. Настройку
        // не убираем и оставляем false осознанно: если дегидратацию ошибок
        // или pending-запросов когда-нибудь включат обратно, эта строка
        // перестанет быть мёртвой и начнёт отдавать в браузер тексты
        // серверных ошибок как есть, без редактирования.
