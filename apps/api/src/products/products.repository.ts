...
export class ProductsRepository {
  ...
  /**
   * Список и счётчик — одной поездкой в базу через $transaction([...]), а не
   * потому что транзакция гарантирует их согласованность: соединение здесь на
   * READ COMMITTED (BEGIN без ISOLATION LEVEL), и каждый из двух операторов
   * берёт собственный снимок. Вставка между findMany и count всё равно может
   * пройти, и total на лету разойдётся со страницей на единицу — решение
   * контроллера осознанно не платит за RepeatableRead (дороже и сериализационные
   * ошибки на каждый список) ради этой редкой расфазировки на витрине каталога.
   */
  async getList({
    pathPrefix,
    offset,
