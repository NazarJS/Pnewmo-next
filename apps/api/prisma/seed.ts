  ...
  try {
    ...
    const { categoriesInserted, productsInserted } = await prisma.$transaction(
      async (tx) => {
        ...
        // таблицы падает в зависимости от порядка строк. Товары стираются не
        // из-за CASCADE, а потому что перечислены в самой команде; сегодня
        // CASCADE здесь no-op и оставлен страховкой на случай, если появится
        // таблица, которая сошлётся на categories или products, но в список
        // не попадёт.
