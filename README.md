...
| Команда | Что делает |
|---|---|
| `pnpm dev` | запускает всё: Postgres, фронтенд, API |
...
- **`db:seed` запускается следом автоматически** — его вызывает сам `pnpm dev`, уже после
  `db:sync`. Это безопасно: `TRUNCATE` внутри сида стоит за сторожевым условием и
  выполняется только когда товаров в базе ещё нет (сразу после миграции или
  `db:reset`). Если данные уже залиты — сидом или руками через API, — скрипт печатает,
  что пропущен, и выходит, ничего не трогая. Принудительно стереть и перезалить —
  `pnpm db:seed:reset`.
...
Порты: фронтенд 3000, API 4000, Postgres 5433.

---

# Схема базы данных

Prisma 7, PostgreSQL 16. Две таблицы: `categories` — дерево через `parent_id`
(adjacency list) с `ON DELETE RESTRICT`, уникальным `slug` и материализованным путём
`path`, вложенность до 5 уровней в сидах; `products` — товар привязан к категории через
`category_id` (тоже `ON DELETE RESTRICT`), характеристики лежат в JSONB. Схема —
`apps/api/prisma/schema.prisma`.

Заказы — следующий этап, см. `docs/superpowers/specs/`.
...
**`json-server` выключен, `db.json` удалён.** Витрина читает категории и товары из
реального API. Панель фильтров временно отцеплена от страницы каталога и ждёт переезда
на реальный API (спека 4c) — код на месте, но не используется.
